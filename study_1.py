...
def main():
    ...
    def weighting_function(return_on_capital, earnings_yield):
        if (return_on_capital < 0) and (earnings_yield < 0):
            weighting = -1 * return_on_capital * earnings_yield
        else:
            weighting = return_on_capital * earnings_yield
    ...
    for year, array in enumerate(yearly_data):
        ...
        for i in range(0, array_rows):
            weighting_column[i, 0] = weighting_function(array[i, 11], array[i,12])
    ...
    print("Final amount is: " + str(round(value_array[len(value_array)-1], 2)))
    print("Volatility of returns: " + str(round(volatility, 3)))
    print("CAGR: " + str(round(CAGR, 2)) + "%")
    
    
    plt.figure()
    plt.title('The growth of $1 invested in return on capital x earnings yield weighted stocks')
